# 👋 Hi there, I'm Vikas Rangi and I'm happy to see here.
Let's connect. 
[![Linkedin Badge](https://img.shields.io/badge/-LinkedIn-0e76a8?style=flat-square&logo=Linkedin&logoColor=white)](https://www.linkedin.com/in/vikas-rangi)
[![Email](https://img.shields.io/badge/Email-D14836?logo=gmail&logoColor=white&style=for-the-badge)](mailto:vikasrangi0328@outlook.com)<br>

Meet me, Vikas Rangi. I'm transitioning into Data Analytics with hands-on experience in Power BI and data modeling. I bring strong communication skills, logical thinking, and real-world business understanding from customer-facing roles.

<img align="right" alt="GIF" src="https://github.com/vikas0328/About_Me/blob/fc8aab504a17163761f65585b3d1f554bda57c65/never-stop-learning-symbol-wooden-260nw-2004893450.jpg" width="400" height="225" />

🎯 Aspiring Data Analyst | Power BI | Power Query | Excel | Data Modeling  

## 🚀 **About Me**  
- 📊 Experienced with **Power BI, Power Query, Dashboards, Analysis, & MIS Reporting**
- 🔭 Transitioning into Data Analytics and building hands-on projects using Power BI, Power Query and Excel.
- 🌱 Learning *SQL, Python, Azure and combining data with AI.*
- 👯 Open to collaborating on **Analytics Dashboards**
- 🎯 Goal: *To develope strong skills in data analysis and transition into AI-focused roles by combining data with artificial intelligence.*  
- ⚡ **Fun fact:** I enjoy playing chess as much as I enjoy cricket.

---

## 💡 Skills & Tools

- 📊 Power BI (Data Modeling, DAX, Dashboard Design)
- 📈 Power Query & Microsoft Excel (Data Analysis, Formulas, Pivot Tables)
- 🧠 Analytical Thinking & Problem Solving
- 📢 Strong Communication & Stakeholder Understanding
- 🧑‍💼 Real-world business understanding from customer-facing roles.

---

## 💼 Projects

- 🛵 **Food-Delivery-App-Dashboard-Project**  
  - Built an interactive dashboard to track orders performance  
  - Used DAX measures for KPIs and trend analysis  
  - Focused on business insights and decision-making

🔹 *More projects coming soon...*  🔹

I am actively working on real-world data projects to showcase my skills.

---

## 🌱 Currently Working On

- SQL
- Advanced DAX in Power BI  
- Data Modeling Best Practices  
- Building Portfolio Projects

---

💬 *Thanks for visiting my GitHub! Let’s connect and grow together.*

